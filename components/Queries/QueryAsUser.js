...
export default function QueryAsUser () {
  ...
  const setMsgs = async () => {
    ...
    if (size(msgdata) > 0 && msgdata.error === undefined) {
      msgdata.map(m => {
        ...
        <Grid.Row>
          <div className='queries-user__queue'>
            ...
            >
              <Column
                field='createAt'
                header={t('queriesUserDataTableDateHeader')}
              ></Column>
              <Column
                field='productName'
                header={t('queriesUserDataTableProductHeader')}
              ></Column>
              <Column
                field='clientUnreadMsg'
                header={t('queriesUserDataTableUnreadMsgHeader')}
              ></Column>
            </DataTable>
          </div>
        </Grid.Row>
